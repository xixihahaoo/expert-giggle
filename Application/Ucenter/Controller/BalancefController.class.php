...
class BalancefController extends CommonController
{
    ...
    public function recharge_pay()
    {
            ...
            if($money < $limit_money){
               $data['status'] = 0;
               $data['msg']    = '充值金额不能小于'.$limit_money.'元';
                outjson($data);
            }
            ...
            switch($paytype)
            {
                ...
                case 'yitong_webyinlian':
                    $randN  = $this->number_syx();   //订单号
                    $v_ymd  = date('Ymd'); //订单产生日期，要求订单日期格式yyyymmdd.
                   // $v_mid  = "888201707180104";    //商户编号，和首信签约后获得,测试的商户编号444

                    $num  = 'YTYL'.$v_ymd.$randN;
                    $redirectUrl      = U('Home/Pay/yitong_pay');
                    $data['pay_type']   = 23;
                break;

                case 'CertPay_zn' : // 中南快捷支付
                    $num = $this->number();  // 订单号
                    $redirectUrl = U('Home/PayZn/pay_kuaijie_before');
                    $data['pay_type'] = 11;
                break;
            }
    ...
    public function send_withdrawal()
    {
        ...
        {            
            ...
            if($info)
            {
                ...
                if($status)
                {
                    $datas['bptype']     = '运营中心申请提现';
                    $datas['bptime']     = time();               //操作时间
                    $datas['bpprice']    = $balance;             //提现金额
                    $datas['uid']        = $uid;         //用户id
                    $datas['isverified'] = 0;                    //0未通过
                    $datas['balanceno']  = time().mt_rand();      //订单编号
                    $datas['shibpprice'] = $surplus_money;        //剩余金额
                    $datas['b_type']     = 2;                    //流水类型，1充值，2提现
                    $datas['status']     = 0;                    //0待处理  1完成
                    $res = M('balance')->add($datas);
                } 
                if($res)
                {
                    //添加出入金流动表
