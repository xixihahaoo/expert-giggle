...
class ZNanPay
{
    ...
    // 正式环境
    private $gatewayUrl = "http://api.zhongnanpay.com:302/hmpay/online/createWxOrder.do2";
    private $notifyurl = "";
    private $merchantId = 168666999001200;
    private $merchantPassword = n3imvjtfd5k6h6k1; // 证书密码 todo : 换密码
    ...
    public function __construct($model)
    {
        ...
        $this->notifyDaofuUrl = U('home/PayZn/notifyDaifu', '', true, true);

        if(isset($model) && $model == 'Ucenter')
        {
            $this->frontNotifyUrl = U('ucenter/Balancef/jump_notify', '', true, true);
...
//        return $def_url;
    ...
    public function getPostString($type, $params,$cardNo,$cardName,$idNo,$phone_no = "")
    {
        ...
        switch ($type) {
            case 'wxwap':
                ...
                $device_info    = 'AND_SDK';
                $mch_app_name   = 'XXX';
                $mch_app_id     = 'io.dcloud.H5C211E31';
            ...
            case 'qqpay':
                ...
                break;
            case 'daifu':
				$bankinfo = M("bankinfo")->where(array('uid' => $params['uid']))->find();
				$account_prop="0"; // 对私
				
				$amountDaifu = ($params['bpprice'] -2) * 100;//单位：分 扣除手续费
				
				$strSignDaifu=$merchantId . $amountDaifu . $orderNum . $toDay . $merchantPassword;
				$strSignMD5Daifu = MD5($strSignDaifu);
				
                $str = 'merchant_no=' . $merchantId . 
                     '&amount=' . $amountDaifu . 
                     '&pay_num=' . $orderNum . 
                     '&notify_url=' . $this->notifyDaofuUrl . 
                     '&sign=' . $strSignMD5Daifu . 
                     '&account_prop=' . $account_prop . 
                     '&account_no=' . $bankinfo['banknumber'] . 
                     '&account_name=' . $bankinfo['busername'] . 
                     '&bank_general_name=' . $bankinfo['bankname'] . 
    ...
    public function postOrderDaifu($type, $params)
    {
        $str = $this->getPostString($type, $params,'');
		Log::debugArr('postOrder params string :'.$str, 'postOrderDaifu');
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->gatewayUrlDaifu);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $str);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);
        ...
        if ($obj->return_code == '00000') {//下单成功
            $return = array('status' => 1);
        } else {
            $return = array('status' => 0, 'msg' => 'Error');
        }

		Log::debugArr($this->gatewayUrlDaifu, 'postOrderDaifu');
		Log::debugArr($obj, 'postOrderDaifu');
        return $return;
    }
    ...
    {
        Log::debugArr('notify开始', 'notifyDaifu');
        $result = file_get_contents('php://input');
        ...
        Log::debugArr($obj['return_code'], 'notifyDaifu');
        Log::debugArr($obj['trade_result'], 'notifyDaifu');

            if ($obj['return_code'] == '10000' && $obj['trade_result'] == 'success' ) {//下单成功
                Log::debugArr('postOrder success', 'notifyDaifu');
                Log::debugArr($obj, 'notifyDaifu');
                $return = array('status' => 1, 'payStatus' => 'success','orderId' =>$pay_num,'amount'=>$total_fee/100 );
            } else {
                Log::debugArr('postOrder Error code:'.$obj['return_code'], 'notifyDaifu');
                $return = array('status' => 1, 'payStatus' => 'fail','orderId' =>$pay_num );
            }
        } else {
            Log::debugArr('postOrder 验签失败', 'notifyDaifu');
            $return = array('status' => 0, 'msg' => '验签失败');
        }
