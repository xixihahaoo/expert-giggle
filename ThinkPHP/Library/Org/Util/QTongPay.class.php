...
class QTongPay
{
    ...
    public function __construct()
    {
        ...
        $this->frontNotifyUrl = U('home/PayQt/jump_notify', '', true, true);
    }
   ...
   public function jumpRequest($params){
       $str = $this->getPostString('Kuaijie',$params);
       Log::debugArr('jumpRequest params string :'.$str, 'qtpay');
        ...
        $def_url .= '<body onLoad="document.ipspay.submit();">正在跳转到支付页面,请稍后...';
        $def_url .= '<form name="ipspay" action="'.$this->gatewayUrl.'" method="post">';
    ...
    public function getPostString($type, $params)
    {
        ...
        switch ($type) {
            ...
            case 'Kuaijie':
                ...
                <message application="CertPayOrderH5"
                guaranteeAmt="0"
                    merchantFrontEndUrl="'.$this->frontNotifyUrl.'"
                    merchantId="' . $merchantId . '"
                    merchantName=""
                    merchantOrderAmt="' . $amount . '"
                    merchantOrderDesc="会员充值"
...
//                header("Location: ".$responseData['codeUrl']);
    ...
    public function notify()
    {
        ...
        if ($this->verity(MD5($resp_xml, true), $resp_sign)) {//验签
            ...
            Log::debugArr('pay xml : '.$resp_xml, 'qtpay_notify');
