...
class demoB2CB2BPayment {
    ...
    public static function bankPayment($data) {
        
        $notifyUrl = UtilHelper::desEncode($data['notifyUrl']);
        $params = [
            'pageReturnUrl' => $data['pageReturnUrl'],   //页面返回地址,String(100),页面重定向URL支付结果同步返回到该url，该url由商户提供
            'notifyUrl' => $notifyUrl,  //后台通知地址,String(100),后台异步通知URL，交易结果 通过后台通知到此url，为确保安全该字段支持 (DES加密，ECB模式)
            'merchantName' => '在线支付',
            'memberId' => $data['memberId'],
            'orderTime' => $data['orderTime'],  //订单提交时间，格式YYYYMMDDHHmmss
            'orderId' => $data['orderId'], 
            'totalAmount' => $data['totalAmount'],    //订单金额，以分为单位，￥35.65，只能传整数
            'currency' => 'CNY',
            'bankAbbr' => $data['bankAbbr'],    //银行简称
